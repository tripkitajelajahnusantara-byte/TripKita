  ...
  Future<void> _create() async {
    if (_store.plans.length >= TripPlanStore.maxPlans) {
      await showNoticeDialog(
        ...
        message: 'Batas maksimal 10 rencana trip telah tercapai. Hapus rencana lama terlebih dahulu jika ingin membuat rencana baru.',
        isError: true,
      );
      return;
    }
    final plan = await Navigator.of(context).push<TripPlan>(MaterialPageRoute(builder: (_) => const TripPlanFormScreen()));
    if (plan != null && mounted) _open(plan);
  }

  void _open(TripPlan plan) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => TripPlanDetailScreen(planId: plan.id)));
  }
            ...
            children: [
              ...
              if (plans.isEmpty)
                EmptyState(
                  ...
                  message: 'Mulai rencanakan liburan impianmu dan pantau progres tabungannya di sini.',
                  actions: [ElevatedButton(onPressed: _create, child: const Text('Buat Rencana Pertama'))],
                )
              else
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Row(children: [
              ...
              Text('$pct%', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: AppColors.primary)),
            ]),
            const SizedBox(height: 6),
            Text('${formatIsoLong(plan.targetDate)} • ${plan.participants} orang',
            ...
            Text('${formatIDR(plan.savedAmount)} dari ${formatIDR(plan.targetBudget)}',
                style: const TextStyle(fontSize: 12.5, color: AppColors.textMedium, fontWeight: FontWeight.w600)),
          ]),
        ),
      ),
    );
...
class _RupiahInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    final text = _format.format(int.parse(digits.length > 13 ? digits.substring(0, 13) : digits));
    return TextEditingValue(text: text, selection: TextSelection.collapsed(offset: text.length));
  }
}

int _parseRupiah(String text) => int.tryParse(text.replaceAll(RegExp(r'\D'), '')) ?? 0;
String _formatRupiahInput(int value) => value <= 0 ? '' : NumberFormat.decimalPattern('id_ID').format(value);
...
class _TripPlanFormScreenState extends State<TripPlanFormScreen> {
  ...
  late final _budget = TextEditingController(text: _formatRupiahInput(widget.existing?.targetBudget ?? 0));
  late DateTime _date = parseIsoDate(widget.existing?.targetDate ?? '') ?? dateOnly(DateTime.now()).add(const Duration(days: 30));
  ...
  Future<void> _submit() async {
    ...
    setState(() {
      _destinationError = destination.isEmpty ? 'Silakan isi destinasi impian Anda.' : null;
      _budgetError = budget <= 0 ? 'Silakan masukkan target budget yang valid.' : null;
    });
    ...
    await TripPlanStore.instance.upsert(plan);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.existing == null ? 'Rencana Trip Baru' : 'Ubah Rencana Trip')),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          SectionCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: const Icon(Icons.location_on_outlined, size: 18, color: AppColors.textLight),
              ...
              InkWell(
                ...
                child: InputDecorator(
                  decoration: const InputDecoration(
                    prefixIcon: Icon(Icons.calendar_today_outlined, size: 18, color: AppColors.textLight),
                  ),
                  child: Text(formatDateLong(_date), style: const TextStyle(fontSize: 14)),
                ),
              ),
              const SizedBox(height: 14),
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text('$_participants Orang', style: const TextStyle(fontWeight: FontWeight.w700))),
                  IconButton(
                    onPressed: _participants > 1 ? () => setState(() => _participants--) : null,
                    icon: const Icon(Icons.remove_circle_outline),
                  ),
                  IconButton(
                    onPressed: _participants < 50 ? () => setState(() => _participants++) : null,
          ...
          PrimaryButton(
            label: widget.existing == null ? 'Buat Rencana Trip' : 'Simpan Perubahan',
...
class _Motivation {
  ...
  const _Motivation(this.icon, this.title, this.text, this.background, this.border, this.titleColor, this.textColor);
}
...
_Motivation _motivationFor(int pct, bool dateReached, String dest) {
  if (dateReached && pct < 100) {
    return _Motivation('🗓️', 'Waktu Keberangkatan Tiba, Tapi Jangan Patah Semangat! 💪',
        ...
        const Color(0xFFFFFBE6), const Color(0xFFFFE58F), const Color(0xFFD48806), const Color(0xFF784C00));
  }
  if (pct >= 100) {
    return _Motivation('🎉', 'Hore! Target Tabunganmu 100% Tercapai! 🎉',
        'Selamat! Tabungan untuk liburan impian ke $dest sudah terkumpul 100%. Yuk langsung cari dan pesan paket trip impianmu! 🚀',
        AppColors.successBg, const Color(0xFF86EFAC), const Color(0xFF15803D), const Color(0xFF166534));
  }
  if (pct >= 75) {
    return _Motivation('💪', 'Hampir Sampai! 75%+ Tabungan Terkumpul! 💪',
        'Ayo semangat! Tinggal sedikit lagi tabunganmu terkumpul 100% untuk liburan impian ke $dest! ✨',
        const Color(0xFFECFDF5), const Color(0xFFA7F3D0), const Color(0xFF047857), const Color(0xFF065F46));
  }
  if (pct >= 25) {
    return _Motivation('🔥', 'Awal yang Bagus! 25% Tabungan Sudah Terkumpul! 🔥',
        'Kerja bagus! Tabungan liburan ke $dest sudah mulai terkumpul. Tetap konsisten menyisihkan tabungan tiap bulan ya! ✨',
        AppColors.accentLight, const Color(0xFFBAE6FD), AppColors.primary, AppColors.primaryDark);
  }
  return _Motivation('🌱', 'Langkah Awal Memulai Perjalanan Impian! 🚀',
      ...
      AppColors.accentSoft, AppColors.accentBorder, AppColors.primary, AppColors.primaryDark);
}
...
class _TripPlanDetailScreenState extends State<TripPlanDetailScreen> {
  ...
  Future<void> _save(TripPlan plan) => _store.upsert(plan);

  Future<void> _editPlan(TripPlan plan) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => TripPlanFormScreen(existing: plan)));
  }
  ...
  Future<void> _deletePlan(TripPlan plan) async {
    final ok = await showConfirmDialog(
      ...
      message: Text('Rencana trip ke "${plan.destination}" beserta seluruh catatan tabungannya akan dihapus.',
  ...
  Future<void> _deleteSavings(TripPlan plan, TripSavingsLog log) async {
    final ok = await showConfirmDialog(
      ...
      message: Text('Catatan ${formatIDR(log.amount)} akan dihapus dari rencana ini.',
          textAlign: TextAlign.center, style: const TextStyle(color: AppColors.textMuted)),
      cancelLabel: 'Batal',
      confirmLabel: 'Hapus',
      confirmColor: AppColors.dangerDark,
    );
    if (ok) await _save(plan.copyWith(savingsLogs: plan.savingsLogs.where((l) => l.id != log.id).toList()));
  }
  ...
  Future<void> _toggleItem(TripPlan plan, TripChecklistItem item) async {
    ...
    await _save(plan.copyWith(
      checklist: plan.checklist.map((c) => c.id == item.id ? c.copyWith(completed: !c.completed) : c).toList(),
    ));
  }
  ...
  Future<void> _addItem(TripPlan plan) async {
    ...
    await _save(plan.copyWith(checklist: [
      ...plan.checklist,
      TripChecklistItem(id: 'chk_${DateTime.now().millisecondsSinceEpoch}', label: label),
    ]));
  }
  ...
  Future<void> _renameItem(TripPlan plan, TripChecklistItem item) async {
    ...
    await _save(plan.copyWith(
      checklist: plan.checklist.map((c) => c.id == item.id ? c.copyWith(label: label) : c).toList(),
    ));
  }

  Future<void> _deleteItem(TripPlan plan, TripChecklistItem item) async {
    await _save(plan.copyWith(checklist: plan.checklist.where((c) => c.id != item.id).toList()));
  }

  void _searchPackages(TripPlan plan) {
    appShellKey.currentState?.openSearch(TripSearchParams(destination: plan.destination));
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        if (plan == null) {
          return Scaffold(appBar: AppBar(), body: const Center(child: Text('Rencana trip tidak ditemukan.')));
        }
        return Scaffold(
          appBar: AppBar(
            title: const Text('Detail Rencana Trip'),
            actions: [
              IconButton(tooltip: 'Ubah', onPressed: () => _editPlan(plan), icon: const Icon(Icons.edit_outlined)),
              IconButton(
                tooltip: 'Hapus',
  ...
  Widget _buildHeader(TripPlan plan) {
    return SectionCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(plan.destination, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppColors.textDark)),
        const SizedBox(height: 8),
        Wrap(spacing: 16, runSpacing: 6, children: [
          Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.calendar_today_outlined, size: 15, color: AppColors.primary),
            const SizedBox(width: 5),
            Text(formatIsoLong(plan.targetDate), style: const TextStyle(fontSize: 13, color: AppColors.textMedium)),
          ]),
          Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.groups_outlined, size: 16, color: AppColors.primary),
            const SizedBox(width: 5),
            Text('${plan.participants} orang', style: const TextStyle(fontSize: 13, color: AppColors.textMedium)),
          ]),
        ]),
      ]),
    );
  ...
  Widget _buildProgress(TripPlan plan) {
    ...
    Widget stat(String label, int value, Color color) => Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            FittedBox(
              ...
              child: Text(formatIDR(value), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: color)),
            ),
          ]),
        );
    ...
    return SectionCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          const Expanded(
            child: Text('📈 Progres Tabungan', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          ),
          Text('$pct%', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: AppColors.primary)),
        ]),
        const SizedBox(height: 12),
        ClipRRect(
          borderRadius: BorderRadius.circular(6),
        ...
        Container(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(m.title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: m.titleColor)),
                const SizedBox(height: 4),
                Text(m.text, style: TextStyle(fontSize: 12.5, color: m.textColor, height: 1.5)),
              ]),
            ),
          ]),
        ),
  ...
  Widget _buildSavings(TripPlan plan) {
    return SectionCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          const Expanded(
            child: Text('💰 Catatan Tabungan', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          ),
          TextButton.icon(
        ...
        if (plan.savingsLogs.isEmpty)
          const Padding(
            ...
            child: Text('Belum ada catatan tabungan. Tambahkan setiap kali kamu menyisihkan uang untuk trip ini.',
        ...
        for (final log in plan.savingsLogs)
          ListTile(
            ...
            leading: const CircleAvatar(
              ...
              child: Icon(Icons.savings_outlined, color: AppColors.success, size: 20),
            ),
            title: Text(formatIDR(log.amount), style: const TextStyle(fontWeight: FontWeight.w800)),
            subtitle: Text('${log.date}${log.note.isNotEmpty ? ' • ${log.note}' : ''}',
                style: const TextStyle(fontSize: 12)),
            trailing: PopupMenuButton<String>(
              onSelected: (v) => v == 'edit' ? _editSavings(plan, log) : _deleteSavings(plan, log),
              itemBuilder: (_) => const [
  ...
  Widget _buildChecklist(TripPlan plan) {
    ...
    return SectionCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          const Expanded(
            child: Text('✅ Checklist Persiapan', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          ),
        ...
        for (final item in plan.checklist)
          ListTile(
            ...
            leading: Icon(
              plan.isCompleted(item) ? Icons.check_circle : Icons.radio_button_unchecked,
              color: plan.isCompleted(item) ? AppColors.success : AppColors.borderStrong,
            ),
            title: Text(item.label,
                style: TextStyle(
                  fontSize: 13.5,
                  color: plan.isCompleted(item) ? AppColors.textMuted : AppColors.textDark,
                  decoration: plan.isCompleted(item) ? TextDecoration.lineThrough : null,
                )),
            subtitle: item.isAutomatic ? const Text('Otomatis', style: TextStyle(fontSize: 11)) : null,
            trailing: item.isAutomatic
                ? null
                : PopupMenuButton<String>(
                    onSelected: (v) => v == 'edit' ? _renameItem(plan, item) : _deleteItem(plan, item),
                    itemBuilder: (_) => const [
                      PopupMenuItem(value: 'edit', child: Text('Ubah')),
                      PopupMenuItem(value: 'delete', child: Text('Hapus')),
                    ],
        ...
        Row(children: [
          Expanded(
            child: TextField(
              ...
              decoration: const InputDecoration(hintText: 'Tambah item, mis. Sewa kamera'),
            ),
          ),
          const SizedBox(width: 8),
          IconButton.filled(
  ...
  Widget _buildPackages(TripPlan plan) {
    return SectionCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const Text('🧭 Paket yang Cocok', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
        const SizedBox(height: 4),
          ...
          builder: (context, snapshot) {
            ...
            final matches = (snapshot.data ?? const <TripPackage>[])
                .where((p) =>
                    ...
                    (p.destination.isNotEmpty && q.contains(p.destination.toLowerCase())))
                .take(4)
                .toList();
            if (matches.isEmpty) {
              return Text(
                  ...
                  SizedBox(
                    ...
                    child: TripGridCard(
                      ...
                      onTap: () => Navigator.of(context)
                          .push(MaterialPageRoute(builder: (_) => TripDetailScreen(package: pkg))),
                    ),
                  ),
              ]);
            });
...
class _SavingsSheetState extends State<_SavingsSheet> {
  late final _amount = TextEditingController(text: _formatRupiahInput(widget.existing?.amount ?? 0));
  ...
  void _submit() {
    ...
    Navigator.pop(
      ...
      TripSavingsLog(
        ...
        date: existing?.date ?? formatDateShort(DateTime.now()),
        amount: amount,
        note: _note.text.trim().isEmpty ? 'Tabungan bulanan' : _note.text.trim(),
      ),
    );
  }
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Text(widget.existing == null ? 'Tambah Tabungan' : 'Ubah Tabungan',
          ...
          TextField(
            ...
            keyboardType: TextInputType.number,
            inputFormatters: [_RupiahInputFormatter()],
            decoration: InputDecoration(prefixText: 'Rp ', hintText: '500.000', errorText: _error),
          ),
          const SizedBox(height: 14),
          const FieldLabel('Catatan', hint: '(opsional)'),
          TextField(controller: _note, decoration: const InputDecoration(hintText: 'Tabungan bulanan')),
          const SizedBox(height: 20),
          PrimaryButton(label: 'Simpan', color: AppColors.primary, onPressed: _submit),
        ]),
      ),
    );
  }
